...
- [x] **IMPORTANT: Command Collision Mitigation**: Renamed `/invsee` and `/endersee` to `/isinvsee` and `/isendersee` to prevent conflicts with server utility mods (Essentials). *Note: vanilla `/ban` collision tracked in `KNOWN_ISSUES.md` §8.*
- [x] **Pre-Join Disconnection**: Refactor punishment login checks to happen during the network login handshake phase (e.g. `ServerLoginPacketListenerImpl`) rather than `PlayerLoggedInEvent`. This prevents banned players from briefly appearing in the world or triggering "Player joined the game" messages before being kicked, which is especially noticeable on integrated servers like Essential.
